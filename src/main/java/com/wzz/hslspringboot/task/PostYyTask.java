...
/**
 * 预约任务统一调度器 (已升级为并发调度模式)
 *
 * @version 2.3 (修改：改为定时并发调度)
 * @description 调度器每秒扫描一次数据库，然后使用虚拟线程池并发处理所有待调度任务的预检和调度逻辑，
 *              解决了在待处理任务量大时串行处理造成的调度延迟问题。
 */
@Component
public class PostYyTask {

    private static final Logger logs = LogManager.getLogger(PostYyTask.class);
    ...
    public void init() {
        ...
        logs.info("虚拟线程预约任务调度器已初始化。");
    }

    /**
     * 每秒执行一次，扫描数据库中“待处理”的任务并【并发】进行调度
     */
    ...
    public void scheduleNewAppointments() {
        ...
        for (UserSmsWebSocket user : pendingTasks) {
            taskProcessorExecutor.submit(() -> processAndScheduleTask(user, yzmTimeS, maxRetries, retriesTime));
        }
    }


    /**
     * 处理并调度单个任务的逻辑单元。此方法将在虚拟线程中并发执行。
     */
    private void processAndScheduleTask(UserSmsWebSocket user, int yzmTimeS, int maxRetries, int retriesTime) {
        ...
        try {
            ...
            log.err(logs, user.getId(),"用户ID: {} 的预约时间'{}'格式无效。", user.getId(), user.getAppointmentTime());
            userSmsWebSocketService.updateTaskStatus(user.getId(), STATUS_INVALID_TIME, "预约时间格式错误: " + e.getMessage());
            return;
        }
        try {
            log.info(logs, user.getId(),"对用户ID: {} 的任务进行预检...", user.getId());
            if (!appointmentProcessorService.preProcessCheck(user)) {
                log.err(logs, user.getId(),"用户ID: {} 的任务未能通过预检，将不会被调度。", user.getId());
                return; // 预检失败，直接返回
            }
            log.info(logs, user.getId(),"用户ID: {} 的任务通过预检（可能包含警告），准备调度。", user.getId());
        } catch (Exception e) {
            log.err(logs, user.getId(),"对用户ID: {} 的任务进行预检时发生严重异常，任务将不会被调度。", user.getId(), e);
        ...
        if (delayMillis < 0) {
            log.err(logs, user.getId(),"用户ID: {} 的任务执行时间已过或过于接近，将立即执行。预约时间: {}", user.getId(), user.getAppointmentTime());
            delayMillis = 0;
        }
        try {
            Future<?> future = scheduler.schedule(() -> {
                // 真正的执行业务交还给虚拟线程池并发执行
                taskProcessorExecutor.submit(() -> executeAppointmentWithRetries(user, maxRetries, retriesTime));
            }, delayMillis, TimeUnit.MILLISECONDS);
            Future<?> existingTask = scheduledTasks.putIfAbsent(user.getId(), future);
            if (existingTask != null) {
                future.cancel(false);
                log.info(logs, user.getId(),"用户ID: {} 的任务已被其他线程抢先调度，本次操作取消。", user.getId());
            } else {
                userSmsWebSocketService.updateTaskStatus(user.getId(), STATUS_SCHEDULED, "任务已进入调度队列，等待执行。");
                log.info(logs, user.getId(),"用户ID: {} 的任务已成功调度，将在约 {} 秒后执行（已提前 {} 秒），最大重试次数: {}。", user.getId(), delayMillis / 1000L, yzmTimeS, maxRetries);
            }
        } catch (Exception e) {
            log.err(logs, user.getId(),"调度用户ID: {} 的任务时发生异常。", user.getId(), e);
            userSmsWebSocketService.updateTaskStatus(user.getId(), STATUS_FAILED, "任务调度失败: " + e.getMessage());
            // 出现异常时，确保从 map 中移除占位，以便下次可以重新调度
    ...
    private void executeAppointmentWithRetries(UserSmsWebSocket user, int maxRetries, int effectiveRetriesTime) {
        if (userSmsWebSocketService.getById(user.getId()) == null) {
            log.err(logs, user.getId(),"任务ID: {} 即将执行时发现其在数据库中已不存在，任务取消。", user.getId());
            scheduledTasks.remove(user.getId());
            return;
        }
        log.info(logs, user.getId(),"开始清空用户【{}】(ID:{}) 的历史执行日志...", user.getUserName(), user.getId());
        userSmsWebSocketService.clearUserLogInfoById(user.getId());
        log.info(logs, user.getId(),"用户【{}】(ID:{}) 的历史日志已清空。", user.getUserName(), user.getId());


        Map<String, Object> preparedData;
        try {
            // ================== 1. 数据准备阶段 (只执行一次) ==================
            log.info(logs, user.getId(),"【阶段1/3】开始为用户ID: {} 准备预约数据...", user.getId());
            ...
            if (dto == null || StrUtil.isBlank(dto.getUuid())) {
                log.err(logs, user.getId(),"用户ID: {} 数据准备阶段成功，但未能获取到(uuid)，这可能导致需要短信验证时提交失败。", user.getId());
            }
            log.info(logs, user.getId(),"【阶段1/3】用户ID: {} 的预约数据准备成功。", user.getId());
        } catch (Exception e) {
            log.err(logs, user.getId(),"用户ID: {} 在[数据准备阶段]失败，任务终止。异常类型: {}", user.getId(), e.getClass().getSimpleName(), e);
        ...
        try {
            ...
            if (now.isBefore(appointmentDateTime)) {
                ...
                if (millisToWait > 0) {
                    log.err(logs, user.getId(),"【阶段2/3】用户ID: {} 等待精准预约时间... 需等待 {} 秒。", user.getId(), millisToWait / 1000L);
            ...
            log.err(logs, user.getId(),"【阶段2/3】用户ID: {} 在等待精准预约时间时被中断。", user.getId(), e);
            userSmsWebSocketService.updateTaskStatus(user.getId(), STATUS_FAILED, "等待提交时间被中断: " + e.getMessage());
            scheduledTasks.remove(user.getId());
            return;
        } catch (Exception e) {
            log.err(logs, user.getId(),"【阶段2/3】用户ID: {} 在等待精准预约时间时发生未知错误。", user.getId(), e);
        ...
        // ================== 3. 提交与重试阶段 ==================
        log.info(logs, user.getId(),"【阶段3/3】用户ID: {} 已到达指定时间，开始执行提交...", user.getId());
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                if (userSmsWebSocketService.getById(user.getId()) == null) {
                    log.err(logs, user.getId(),"任务ID: {} 在重试过程中发现其在数据库中已不存在，停止重试。", user.getId());
                    scheduledTasks.remove(user.getId());
                    return;
                }
                ...
                if(attempt==1){
                    ...
                    o = appointmentProcessorService.submitAppointment(headersToSubmit, dtoToSubmit,user,2);
                }
                if (o != null && o.getJSONObject("data").getInteger("retCode")==1) {
                    log.info(logs, user.getId(),"用户ID: {} 的预约任务提交成功。API响应: {}", user.getId(), o.toJSONString());
                    userSmsWebSocketService.updateTaskStatus(user.getId(), STATUS_SUCCESS, "预约流程执行成功。");
                    scheduledTasks.remove(user.getId());
                    return;
                }
                userSmsWebSocketService.updateTaskStatus(user.getId(), STATUS_PROCESSING, String.format("第 %d/%d 次尝试提交出现错误：%s", attempt, maxRetries,o.toString()));

                String errorMsg = (o != null) ? o.getString("msg") : "返回结果为null或业务失败";
                throw new BusinessException(0,errorMsg);

            } catch (Exception e) {
                log.err(logs, user.getId(),"用户ID: {} 的预约任务第 {}/{} 次提交失败。错误详情: {}", user.getId(), attempt, maxRetries, e);
                if (attempt < maxRetries) {
                    ...
                    try {
                        ...
                        Thread.currentThread().interrupt();
                        log.err(logs, user.getId(),"重试等待时线程被中断，任务ID: {}，将停止重试。", user.getId());
                        userSmsWebSocketService.updateTaskStatus(user.getId(), STATUS_FAILED, "重试等待被中断");
                        scheduledTasks.remove(user.getId());
                        return;
                    }
                } else {
                    log.err(logs, user.getId(),"用户ID: {} 的任务在尝试 {} 次提交后最终失败。", user.getId(), maxRetries);
    ...
    private int parseConfigInt(Map<String, Object> configValue, String key, int defaultValue) {
        ...
        try {
            ...
            logs.error("解析系统配置 '{}' 失败，将使用默认值 {}。错误: {}", key, defaultValue, e.getMessage());
        }
        return defaultValue;
    }
